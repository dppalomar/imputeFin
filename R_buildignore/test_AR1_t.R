...
estimation_result <- estimateAR1t(y, random_walk = FALSE, zero_mean = FALSE, n_chain = 10, n_thin = 1, 
                         n_iteration = 100, K = 30, output_iterates = TRUE)
...
imputation_result_gaussian <- imputeAR1Gaussian(y, n_sample = 1)

layout(matrix(c(1, 2)), heights = c(1, 1))
par(mar=c(4, 5, 4, 5))
plot(1:n, imputation_result, type = 'o', pch = 19,  ylab = 'y')
points(index_miss, imputation_result[index_miss, 1], col = 'red', pch = 19)
plot(1:n, imputation_result, type = 'o', pch = 19,  ylab = 'y')
points(index_miss, imputation_result_gaussian$y_imputed[index_miss, 1], col = 'red', pch = 19)
