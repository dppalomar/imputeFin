...
We compare our package with the existing package `imputeTS`. We first download the adjusted prices of the S&P 500 index from 2012-01-01 to 2015-07-08, compute the log-prices, and intentionally delete some of them for illustrative purposes. 
```{r, message=FALSE}
# download data
library(quantmod)
...
Now we plot the imputed time series obtained by functions in package `imputeTS` and our function `impute_AR1_t()`:
...
y_imputed1 <- res$y_imputed.1
y_imputed2 <- res$y_imputed.2
y_imputed3 <- res$y_imputed.3


library(gridExtra)
p11 <- plot_imputed(y_orig, title = "Original")
p12 <- plot_imputed(res$y_imputed.1, title = "Imputation 1")
p21 <- plot_imputed(res$y_imputed.2, title = "Imputation 2")
p22 <- plot_imputed(res$y_imputed.3, title = "Imputation 3")
