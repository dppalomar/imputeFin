...
# download data
library(quantmod)
y_orig <- log(Ad(getSymbols("^GSPC", 
                            from = "2012-01-01", to = "2015-07-08", auto.assign = FALSE)))
T <- nrow(y_orig)
...
```{r, message=FALSE, fig.width = 9, fig.height = 7.5, out.width = "100%"}
# impute using packages zoo and imputeTS
library(zoo)
library(imputeTS)
y_imputed_locf   <- zoo::na.locf(y_missing)
y_imputed_linear <- zoo::na.approx(y_missing)
y_imputed_ma     <- imputeTS::na.ma(y_missing)
y_imputed_spline <- imputeTS::na.interpolation(y_missing, "spline")
y_imputed_stine  <- imputeTS::na.interpolation(y_missing, "stine")
y_imputed_kalman <- imputeTS::na.kalman(y_missing)
