...
vignette: |
  %\VignetteIndexEntry{Imputation of Financial Time Series} %\VignetteKeyword{imputation, time series, AR, random walk} %\VignetteEncoding{UTF-8} %\VignetteEngine{knitr::rmarkdown}
---
...
for imputation of missing values in time series that fit an autoregressive (AR)
model and also the parameter estimation of AR model from from incomplete time series with missing values. 
...
We compare our package with the existing package `imputeTS`. We download the adjusted price of S&P 500 index from 2012-01-01 to 2015-07-08, and compute the log prices. Some log prices are deleted to generate missing values. Then we plot the imputed time series obtained by functions in package `imputeTS` and our function `imputeAR1t()`，respectively.
