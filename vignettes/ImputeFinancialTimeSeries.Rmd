...
vignette: |
  ...
  %\VignetteKeyword{imputation, time series, missing values, AR, random walk, heavy tail, Student t, econometrics}
...
names(res)
y_imputed1 <- res$y_imputed.1
plot_imputed(y_imputed1)
```
...
## Imputation
For the imputation, we impute the missing values by randomly drawing
samples from the conditional distribution $p\left(\mathbf{y}_{\mathsf{miss}}|\mathbf{y}_{\mathsf{obs}},\boldsymbol{\theta}^{\mathsf{ML}}\right)$,
where $\mathbf{y}_{\mathsf{obs}}$ is the observed values and $\boldsymbol{\theta}^{\mathsf{ML}}$
is the MLE result for the model parameters. Due to the complexity
of the conditional distribution $p\left(\mathbf{y}_{\mathsf{miss}}|\mathbf{y}_{\mathsf{obs}},\boldsymbol{\theta}^{\mathsf{ML}}\right)$,
we cannot sample from it direcly, and a Gibbs sampling scheme is designed to draw realizations. 
