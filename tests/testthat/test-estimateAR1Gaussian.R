context("Function \"estimateAR1Gaussian()\"")
...
test_that("error control works",{
  expect_error(estimateAR1Gaussian(y = median), "\"y\" must be coercible to a vector or matrix.")
  expect_error(estimateAR1Gaussian(y = "Hongkong"), "\"y\" only allows numerical or NA values.")
  expect_error(estimateAR1Gaussian(y = c(1, 2, NA, 3, 4, NA)), "Each time series in \"y\" must have at least five observations.")
  expect_error(estimateAR1Gaussian(y = y_missing_numeric, tol = -0.1), "\"tol\" must be greater than 0.")
  expect_error(estimateAR1Gaussian(y = y_missing_numeric, maxiter = -2), "\"maxiter\" must be greater than 1.")
}) 


test_that("default mode works", {
  # estimation_Gaussian_uni_check <- estimateAR1Gaussian(y_missing_numeric[,1])
  # estimation_Gaussian_multi_check <- estimateAR1Gaussian(y_missing_numeric)
...
test_that("time series with zero mean works", {
  # estimation_Gaussian_zero_mean_check <- estimateAR1Gaussian(y_missing_numeric[,1], zero_mean = TRUE)
  # save(estimation_Gaussian_zero_mean_check, file = "estimation_Gaussian_zero_mean_check.RData", version = 2, compress = "xz")
  
  estimation_Gaussian_zero_mean <- estimateAR1Gaussian(y_missing_numeric[,1], zero_mean = TRUE)
...
test_that("time series following random walk works", {
  # estimation_Gaussian_random_walk_check <- estimateAR1Gaussian(y_missing_numeric[,1], random_walk = TRUE)
  # save(estimation_Gaussian_random_walk_check, file = "estimation_Gaussian_random_walk_check.RData", version = 2, compress = "xz")
  # 
  estimation_Gaussian_random_walk <- estimateAR1Gaussian(y_missing_numeric[,1], random_walk = TRUE)
